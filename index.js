...
function generateBall() {
  ...
  const ageIndicator = document.createElement('p');
  let age = 0;
  ageIndicator.innerHTML = 3 - age;
  setTimeout(function() {
    age = 1;
    ageIndicator.innerHTML = 3 - age;
  },1000);
  ...
  ball.addEventListener('mouseenter', function() {
    ...
    if(age != 3) {
      ...
      showDeath(name + deathMessage[Math.floor(Math.random() * deathMessage.length)] );
    ...
    const randomSound = Math.floor(Math.random() * oohArr.length);
...
function showDeath(string){
  const out = document.getElementById('hidden-on-load');
