...
pumpButton.addEventListener("click", function() {
    if (pumpState == false && juiceLevel >= 5 && skeletonLevel != 3) {
        pumpButton.style.backgroundColor = "sienna";
        switchSound.play();
        pumpState = true;
        const randomWaitTime = Math.floor(Math.random() * 6)*500 + 500;

        let attempts = 1 + Math.floor(Math.random() * 6)
      
        console.log(attempts);
        const interval = setInterval(function() {
            juiceLevel -= juicePerAttempt;
            failSound.play();
            document.getElementById("juice").style.height = 265*juiceLevel/100 + "px";
            document.getElementById("juiceLevel").innerHTML = juiceLevel;
            attempts--;
            if(attempts == 0){
              pumpButton.style.backgroundColor = "rgb(255, 102, 0)";
              skeletonLevel++;
              waterArr[Math.floor(Math.random() * 3)].play();
              document.getElementById("skeletonJuice").style.height = 150*skeletonLevel/3 + "px";
              document.getElementById("skeletonLevel").innerHTML = skeletonLevel;
              pumpState = false;
              clearInterval(interval);
            }
            if(juiceLevel < juicePerAttempt){
              pumpState = false;
              pumpButton.style.backgroundColor = "rgb(255, 102, 0)";
              clearInterval(interval);
            }
...
skeletonButton.addEventListener("click", function() {
  if(skeletonLevel == 3 && ballCount < maxBalls){
  ...
  ball.addEventListener('mouseenter', function(/*event*/) {
    ...
    */
    ...
    if(stage != 3) {
      ...
      msg.style.color = "red";
      msg.innerHTML = name + deathMessage[Math.floor(Math.random() * deathMessage.length)];
      scroll(msg);
    ...
    if(distressedBall == "" && Math.random() < 0.2){
      distressedBall = name;
      ballInDistress(distressedBall);
    }
    totalBalls++;
    document.getElementById("ballCounter").innerHTML = "Balls: " + totalBalls;
    ball.style.left = (randomX + 100*((Math.random() * 2)-1) )+ "px";
    ball.style.bottom = (randomY - 500) + "px";
  });
}
...
function ballInDistress(name){
  ...
  denyLink.innerHTML = "Kill";
  msg.style.display = "inline-block";
  msg.innerHTML = name + ": AHHHHHH!!! My back- it's broken! Put me out of my misery! (15s)";
  ...
  const interval = setInterval(function() {
    seconds--;
    msg.innerHTML = name + ": AHHHHHH!!! My back- it's broken! Put me out of my misery! (" + seconds + "s)";
    if(seconds == 0){
      msg.innerHTML = name + ": AHHHHHH!!! My back- it's broken! Put me out of my misery!";
      const response = document.createElement('p');
      response.style.color = "red";
      response.innerHTML = name + " succumbed to their injuries";
      scroll(response);
      totalBalls--;
      document.getElementById("ballCounter").innerHTML = "Balls: " + totalBalls;
      clearInterval(interval);
  ...
  helpLink.addEventListener('click', function() {
    msg.innerHTML = name + ": AHHHHHH!!! My back- it's broken! Put me out of my misery!";
    const response = document.createElement('p');
    response.innerHTML = name + ": Oh my Ball, my back is fixed, thank you!";
    scroll(response);
    clearInterval(interval);
    helpLink.remove();
    denyLink.remove();
  ...
  denyLink.addEventListener('click', function() {
    ...
    const response = document.createElement('p');
    response.style.color = "red";
    response.innerHTML = name + " was put down";
    scroll(response);
    totalBalls--;
    document.getElementById("ballCounter").innerHTML = "Balls: " + totalBalls;
    clearInterval(interval);
