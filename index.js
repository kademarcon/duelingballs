...
const juiceButton = document.getElementById("juiceMeter");
var pumpState = false;
/*
var game.skeletonLevel = 0;
var game.juiceLevel = 100;
let totalLiveBalls = 0, game.totalDeadBalls = 0, game.totalBalls = 0;
let game.juicePerAttempt = 5;
*/
function sound(src) {
...
const failSound = new Audio("./sound/thud.wav");
...
const slap1 = new Audio("./sound/slap1.wav");
const slap2 = new Audio("./sound/slap2.wav");
const slap3 = new Audio("./sound/slap3.wav");
const slap4 = new Audio("./sound/slap4.wav");
const slap5 = new Audio("./sound/slap5.wav");
const slap6 = new Audio("./sound/slap6.wav");
const slapArr = [slap1, slap2, slap3];
...
const deepwhoo = new sound("./sound/deepwhoo.wav");
...
pumpButton.addEventListener("click", function() {
    if (pumpState == false && game.juiceLevel >= 5 && game.skeletonLevel != 3) {
        ...
        const interval = setInterval(function() {
            ...
            document.getElementById("juiceLevel").innerHTML = game.juiceLevel;
            attempts--;
            if(attempts == 0){
              pumpButton.style.backgroundColor = "rgb(255, 102, 0)";
              game.skeletonLevel++;
              waterArr[Math.floor(Math.random() * 3)].play();
              document.getElementById("skeletonJuice").style.height = 150*game.skeletonLevel/3 + "px";
              document.getElementById("skeletonLevel").innerHTML = game.skeletonLevel;
              pumpState = false;
...
skeletonButton.addEventListener("click", function() {
  if(game.skeletonLevel == 3 && ballCount < maxBalls){
    generateBall();
    game.skeletonLevel = 0;
    document.getElementById("skeletonJuice").style.height = "0px";
    document.getElementById("skeletonLevel").innerHTML = game.skeletonLevel;
  }
  else{
    failSound.load();
...
function generateBall() {
  if(ballCount == maxBalls) return; //only here for developer make ball
  deepwhoo.play();
  ...
  const container = document.getElementById('spawnDiv');
  ...
  const stageIndicator = document.createElement('p');
  let stage = 1;
  stageIndicator.innerHTML = stage;
  setTimeout(function() {
    stage++;
    stageIndicator.innerHTML = stage;
  },2500);
  ...
  ball.addEventListener('mouseenter', function(/*event*/) {
    ...
    /*
        ...
        const ballX = ballRect.width/2 + ballRect.left;
    const ballY = ballRect.height/2 + ballRect.top;
    const mouseX = event.clientX;
    const mouseY = event.clientY
    let angle = 0;
    if(mouseX > ballX){
      angle = Math.PI+Math.atan(-(ballY - mouseY)/(ballX - mouseX));
    } else {
      angle = Math.atan(-(ballY - mouseY)/(ballX - mouseX));
    }
    ball.style.left = (randomX + 100*Math.cos(angle)) + "px";
    ball.style.bottom = (randomY + 100*Math.sin(angle)) + "px";
    */
    ...
    /*
    ball.style.transition = 'opacity 0.9s';
    ...
    */
    ...
    if(stage != 3) {
      ...
      msg.style.color = "red";
      addMessage(msg, name + deathMessage[Math.floor(Math.random() * deathMessage.length)] );
      killBall();
      ball.style.transition = 'opacity 0.1s ease';
      ball.style.opacity = '0';
      return;
    }

    if(distressedBall == "" && Math.random() < 0.2 && game.totalBalls > 50){
      distressedBall = name;
      ballInDistress(distressedBall);
    }
...
function ballInDistress(name){
  scream.play();
  const distressMessage = name + ": AHHHHHH!!! My back- it's broken! Put me out of my misery!";

  const messageElement = document.createElement('p');
  const helpLink = document.createElement('p');
  const denyLink = document.createElement('p');
  const response = document.createElement('p');
  messageElement.style.display = "inline-block";
  helpLink.classList.add('helpLink');
  denyLink.classList.add('denyLink');

  addMessage(messageElement, distressMessage + " (15s)");
  addMessage(helpLink, "Help");
  addMessage(denyLink, "Kill");
  ...
  function updateCountdown() {
    secondsRemaining--;
    messageElement.innerHTML = distressMessage + " (" + secondsRemaining + "s)";

    // If countdown goes to 0, ball dies
    if(secondsRemaining == 0){
      response.style.color = "red";
      killBall();
      // Allows for event to happen again
      distressedBall = "";

      endDistress(name + "  succumbed to their injuries");
    }
  }

  // Help ball on click help
  helpLink.addEventListener('click', function() {
    endDistress(name + ": Oh my Ball, my back is fixed, thank you!");
  });

  // Kill ball on click kill
  denyLink.addEventListener('click', function() {
    response.style.color = "red";
    killBall();

    endDistress(name + " was put down");
  });

  function endDistress(string){
    addMessage(response, string);
    messageElement.innerHTML = distressMessage;
    helpLink.remove();
    denyLink.remove();
...
function loadProgress() {
  ...
  if (savedState) {
    game = JSON.parse(savedState); // Convert string back to object
    loadEverything();
      alert(`Game progress loaded: ${JSON.stringify(game)}`);
...
function loadEverything(){
  ...
  document.getElementById("skeletonJuice").style.height = 150*game.skeletonLevel/3 + "px";
  document.getElementById("skeletonLevel").innerHTML = game.skeletonLevel;
  document.getElementById("liveBallCounter").innerHTML = "Live: " + game.totalLiveBalls;
  document.getElementById("deadBallCounter").innerHTML = "Dead: " + game.totalDeadBalls;
  document.getElementById("liveBallCounter").innerHTML = "Live: " + game.totalLiveBalls;
}
...
// adds element to chatbox from the element and string of what it says
function addMessage(element, string){
  const out = document.getElementById("chatBox");
  const isScrolledToBottom = out.scrollHeight - out.clientHeight <= out.scrollTop + 1; 
  // add the element
  element.innerHTML = string;
  out.appendChild(element);
  // stays scrolled to the bottom if scrolled to the bottom, otherwise stay where you were scrolled
  if(isScrolledToBottom) out.scrollTop = out.scrollHeight - out.clientHeight;
}

function addBall(){
  game.totalBalls++;
  document.getElementById("totalBallCounter").innerHTML = "Total: " + game.totalBalls;
  game.totalLiveBalls++;
  document.getElementById("liveBallCounter").innerHTML = "Live: " + game.totalLiveBalls;
}
